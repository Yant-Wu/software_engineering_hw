...
3. **B1243005 曹哲維**
	- 任務 1.4：60–80 小時  
	  - 負責設計與實作「AI 推薦引擎」的核心模組。  
	  - 需根據使用者輸入的健康目標（例如：減重、增肌、控制血糖）、飲食偏好、過敏史與身體狀況等資料，定義合適的演算法流程與規則。  
	  - 與資料庫與前端介面協作：從用戶資料與餐食資料庫取得必要資訊，計算出每日與每餐的建議菜單與營養配比，並提供可供前端呈現的標準化輸出格式（例如：JSON API 回傳結構）。  
	  - 需考量可維護性與擴充性，讓未來可加入更多健康指標（如血壓、運動量）或新的推薦策略（例如：時間區段飲食建議）。  
	- 任務 1.10：48–64 小時  
	  - 負責開發「飲食趨勢分析」與「改善建議生成」的邏輯模組。  
	  - 需從使用者的歷史飲食紀錄與圖表數據（例如：每日總熱量、蛋白質／脂肪／碳水比、蔬菜攝取量等）中萃取趨勢與異常情況。  
	  - 根據分析結果產出具體的建議文字（例如：提醒使用者減少含糖飲料、增加高纖食物、調整晚餐份量），並以可重複使用的規則或策略引擎形式實作，便於日後維護與規則調整。  
	  - 需確保建議內容與健康目標一致，並避免與過敏史或禁忌食材相衝突。
